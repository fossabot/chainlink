...
func TestCreateLogConsumption_Happy(t *testing.T) {
	...
	tests := []struct {
		...
		{"different blockhash", cltest.NewHash(), logConsumption1.LogIndex, logConsumption1.ConsumerID},
		{"different log", logConsumption1.BlockHash, 1, logConsumption1.ConsumerID},
...
func TestCreateLogConsumption_Errors(t *testing.T) {
	...
	tests := []struct {
		...
		{"duplicate record", logConsumption1.BlockHash, logConsumption1.LogIndex, logConsumption1.ConsumerID},
	}
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			logConsumption2 := models.LogConsumption{
